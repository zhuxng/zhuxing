...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    NSUInteger offset = scrollView.contentOffset.x / scrollView.bounds.size.width + 0.5;
...
//    if (scrollView.contentOffset.x < _scrollX) //向左滚动
//    {
        if (offset == 0) {
            scrollView.contentOffset = CGPointMake(imageCount * scrollView.bounds.size.width, 0);
        }
        if (offset < _originOffset) {
...
//    }
//    else if (scrollView.contentOffset.x > _scrollX) //向右滚动
//    {
        if (offset == INT16_MAX) {
            scrollView.contentOffset = CGPointMake(imageCount * scrollView.bounds.size.width, 0);
        }
...
{
    _originOffset = scrollView.contentOffset.x / scrollView.bounds.size.width + 0.5;
